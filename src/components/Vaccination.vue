<template>
     ...
     <div class="container">
       ...
       <div class="row">
           ...
           <a href="javascript:void(0)" @click="stop()" class="margin-left-20">Pause</a>
           <a href="javascript:void(0)" @click="resume()" class="margin-left-20">Resume</a>
       </div>
       <div class="row form-group"  v-if="!this.phoneSubmitted">
          <input type="text" class="form-control-inline" name="phone" inputmode="numeric" v-model="phoneNumber" placeholder="phone number"/>
          <button @click="handlePhoneSubmit()" class="btn-primary">Submit</button>
       </div>
       <div class="row form-group" v-if="phoneSubmitted">
        ...
        <button @click="schedule()">Schedule</button>
         ...
         <div class="col col-4">
            ...
            <p class="message" v-for="(message, mIndex) in messages" :key="`message_${mIndex}`">{{ message }}</p>
        </div>
         </div>
       </div> 
       ...
       <div  class="row" v-if="matches && matches.length > 0">
         <div class="row matching">
           <h5>Found {{matches.length}} centers with vacines and trying to book</h5>
           <table class="table table-striped">
             <thead>
                <tr>
                <th>
            ...
            <tbody>
               <tr v-for="(match, index) in matches" :key="`matched_${index}`">
                 <td ><p>{{ match.center_id }}</p></td>
                 <td ><p>{{ match.session_id }}</p></td>
                <td ><p>{{ match.name }}</p></td>
             <td ><p>{{ match.address }}</p></td>
             <td ><p>{{ match.vaccine }}</p></td>
             <td ><p>{{ match.age_limit }}</p></td>
             <td ><p>{{ match.capacity }}</p></td>
...
export default class Vaccination extends Vue {
   ...
   async generateOtp() {
     const response = await this.vaccinationService.generateOtp(this.phoneNumber)
   ...
   get prefferedSlot() {
     const mathingSession = this.matches && this.matches[0];
      if(mathingSession && mathingSession.slots && mathingSession.slots.length > 0) {
   ...
   get isTokenExpired() {
    ...
    async handleOnComplete() {
      ...
      const tokenResponse = await service.validateOtp(this.otpValue, this.transactionId)
        if(tokenResponse && tokenResponse.data) {
          this.token = tokenResponse.data.token;
          this.displayMessage(`Token fetched: ${this.token}`);
          if(!this.participants) {
            const presponse = await this.vaccinationService.getParticipants(this.token);
    ...
    performBooking() {
      this.timer = setInterval(async () => {
         ...
         if(this.isTokenExpired) {
           this.displayMessage('token expired. Please enter otp again and press submit to proceed.');
           this.otpValue = '';
           ...
           clearInterval(this.timer);
            ...
            if(response && response.data) {
              if(response.data.length === 0) {
                ...
                const captchResponse = await this.vaccinationService.getCaptcha(this.token);
    ...
    async schedule() {
      const mathingSession = this.matches && this.matches[0];
      const beneficiaries:string[] = this.participants && this.participants.map(x=> x.id) || [];
      ...
      const scheduleResponse = await this.vaccinationService.scheduleSlot(this.token, this.captchaText, Number(mathingSession.center_id),mathingSession.session_id,this.prefferedSlot ,beneficiaries);
