...
<div>
 <table>
        <thead>
            <tr>
               <th v-if="states && states.length > 0">State</th> 
        ...
        <tbody>
            <tr>
                  <td v-if="states && states.length > 0">
                      <v-select v-model="activeState" name="states" 
                      :options="states" label="state_name" 
                      @option:selected="stateChange"></v-select></td>
                <td v-if="district && district.length > 0"><v-select v-model="activeDistrict" name="district" :options="district" label="district_name" @option:selected="districtChage"></v-select></td>
                    <td ><v-select :options="age" 
                    v-model="currentAge" 
    ...
    <div><p class="alert alert-danger" v-if="errorMessage">{{ errorMessage }}</p></div>
</div>
...
export default class Preferences extends Vue {
    ...
    currentAge = 18;
    activeDistrict: any = { district_id: 363, district_name: 'Pune'};
    ...
    mounted() {
        ...
        if(preference) {
            this.preferences = JSON.parse(preference);
            this.currentAge = this.preferences.age;
        ...
        if(!statesString) {
            this.vaccinationService.getStates().then(x=> {
                if(x && x.data && x.data.states) {
                    this.states = x.data.states;
            ...
            this.states = JSON.parse(statesString);
        }
    ...
    districtChage() {
         ...
         this.vaccinationService.getAllCenters(this.activeDistrict.district_id).then((x:any)=> {
            if(x && x.data) {
                this.centers = x.data;
                this.selectedCenters = [];
            }
    ...
    stateChange() {
        this.vaccinationService.getDistrict(this.activeState.state_id).then(x=> {
           
            if(x && x.data && x.data.districts) {
                this.district = x.data.districts;

                if(this.activeState.state_id !== 21) {
                    this.activeDistrict = x.data.districts[0];
                    this.$forceUpdate();
                } else {
                    this.activeDistrict = {
                        district_id : 363,
    ...
    savePreferences() {
        ...
        this.$emit('preferenceChange', preferences);
    }
}
</script>
